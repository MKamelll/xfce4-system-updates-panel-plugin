...
#define DEFAULT_PANEL_ICON_SIZE 12
...
void sample_save(XfcePanelPlugin *plugin,
                 SamplePlugin *sample)
{
  XfceRc *rc;
  gchar *file;
...
sample_new(XfcePanelPlugin *plugin)
{
  ...
  /* some sample widgets */
  image = gtk_image_new_from_icon_name(DEFAULT_PANEL_ICON_NAME, GTK_ICON_SIZE_MENU);
  gtk_image_set_pixel_size(GTK_IMAGE(image), DEFAULT_PANEL_ICON_SIZE);
  gtk_widget_show(image);
  gtk_box_pack_start(GTK_BOX(sample->hvbox), image, FALSE, FALSE, 0);
...
static void
on_count_of_available_updates_finished(GObject *source, GAsyncResult *result, gpointer user_data)
{
  GtkLabel *label = GTK_LABEL(user_data);
  GError *error = NULL;
  gchar *stdout_buff = NULL;

  if (g_subprocess_communicate_utf8_finish(G_SUBPROCESS(source), result, &stdout_buff, NULL, &error))
  {
    gtk_label_set_text(label, stdout_buff ? g_strchomp(stdout_buff) : "no output");
  }
  else
  {
...
static void
count_of_available_updates(GtkLabel *label)
{
  GError *error = NULL;

  GSubprocess *proc = g_subprocess_new(
      G_SUBPROCESS_FLAGS_STDOUT_PIPE, &error,
      "sh", "-c", "zypper list-updates | wc -l", NULL);
  ...
  g_subprocess_communicate_utf8_async(proc, NULL, NULL, on_count_of_available_updates_finished, label);
}

static void
on_system_update(GObject *source, GAsyncResult *result, gpointer user_data)
{
  GError *error = NULL;
  GtkLabel *label = GTK_LABEL(user_data);

  if (g_subprocess_communicate_utf8_finish(G_SUBPROCESS(source), result, NULL, NULL, &error))
  {
...
static gboolean
on_plugin_click(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
  GError *error = NULL;
  GtkLabel *label = GTK_LABEL(user_data);
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 1)
  {
    ...
    GSubprocess *proc = g_subprocess_new(
        G_SUBPROCESS_FLAGS_STDOUT_PIPE, &error,
        "exo-open", "--launch", "TerminalEmulator", "sudo zypper dup", NULL);
    ...
    g_subprocess_communicate_utf8_async(proc, NULL, NULL, on_system_update, label);
  }
...
sample_construct(XfcePanelPlugin *plugin)
{
  ...
  g_signal_connect(sample->ebox, "button-press-event", G_CALLBACK(on_plugin_click), label);
